...
\begin{document}
    \begin{center}
        \framebox{
            \vbox{
                \hbox to 5.78in { {\bf CSCE 411: Design and Analysis of Algorithms (Honors), Fall 2026} \hfill  }
                \vspace{2mm}
                \hbox to 5.78in { {\Large \hfill Exam 1, Section 200 \hfill} }
                %                \vspace{2mm}
    ...
    \paragraph{Fast Fourier Transform (FFT).} Computes the discrete Fourier transform to efficiently convert polynomials between coefficient and point-value representations. Evaluates a polynomial $A(x) = \sum_{j=0}^{n-1} a_j x^j$ at the $n$-th complex roots of unity $\omega_n^k = e^{2\pi i k / n}$. The strategy relies on separating the polynomial into even and odd terms: $A(x) = A_{\mathrm{even}}(x^2) + xA_{\mathrm{odd}}(x^2)$. Interpolation (the Inverse FFT) recovers the original coefficients by evaluating at $\omega_n^{-1}$ and scaling by a factor of $1/n$.
    ...
    \begin{itemize}
        \item $\sum_{i = 0}^n \frac{1}{2^i} \leq 2$, \hspace{1in} $\sum_{i = 1}^n i = \frac{n(n+1)}{2}$
        \item \textbf{Summation Lemma:} For any integer $n \geq 1$ and non-zero integer $k$ not divisible by $n$, $\sum_{j=0}^{n-1} (\omega_n^k)^j = 0$.
    \end{itemize}
